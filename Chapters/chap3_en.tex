...
Indeed, note that in condition~\ref{it3} we speak of the continuity of the map $\fip_j\circ \fip_i^{-1}$, which is well-defined because it is a map between $\re^n$ and $\re^n$. Similarly, we can speak of the differentiability of these maps.
...
but a physical necessity: it is not the same for a car to travel the road Córdoba--Carlos Paz at 10 km/h as at 100 km/h,
...
a coordinate system, that is $\ve x^i(f) = \derp{f}{x^i}$, etc.
...
{\sl take the derivative of a tensor}. 
