%%ultima modificación 26/04/2012
...
Hay dos motivos principales que justifican el estudio 
...
imprecisos un
conjunto de puntos que localmente es como $\re^n$, pero que no
lo es necesariamente en su forma
global. 
...
$p$ y $q\,\in\;M$ luego: o pertenecen al dominio de una misma carta 
...
tiene la estructura de un espacio vectorial y es llamado el espacio
tangente al punto $p$. En efecto, podemos definir la suma de dos
...
\defi: Un {\bf grupo monoparamétrica de difeomorfismos} $g^t$ es un mapa
...
coordenada $x^2$, es decir $V(x)=x^2\frac d{dx}$. La ecuación
...
todo $\re$ y por lo tanto no es un difeomorfismo.
...
$\ve{v}(f)\in \re$. Pero dados $\ve{v}_1$ y $\ve{v}_2 \;\in T_p$\ \ \
$(\ve{v}_1+\ve{v}_2)(f)= \ve{v}_1(f)+ \ve{v}_2(f)$ y por lo tanto $f$ 
define una funcional
lineal $df\mid_p:T_p\to \re$, llamada la diferencial de $f$, es
decir un elemento de $T_p^*$. De esta forma la diferencial de una
función $df$ es un co-vector que al actuar sobre un vector $\ve{v}$ nos
da el número {\it la derivada de $f$  en el punto $p$ en la dirección  de $\ve{v}$}.



Considere el subconjunto $S$ de $M$ tal que $f(M)=\mbox{cte.}$ 
Esta será una  subvariedad
de $M$, es decir superficies embebidas en $M$, de dimensión $n-1$.
La condición $df|_p(\ve{ v})=0$ en vectores de $T_p$ con $p\in S$
significa que estos son en realidad vectores tangentes a $S$, es
decir elementos de $T_p(S)$. Por el contrario, si $df(\ve{v})|_p \neq
0$ entonces en ese punto $\ve{v}$ {\it pincha} a $S$. 

\ejem: La función $f(x,y,z)=x^2+y^2+z^2$ en $\re^3$. \par
$S=\{(x,y,z)\in \re^3 / f(x,y,z) = a^2\;,\,a^2>0\}$ es la esfera de radio
$a$, y como ya hemos visto  una variedad. Sea $(v_x,v_y,v_z)$ un vector
en $(x,y,z)\in\re$, luego la condición $df(v)=2(xv_x+yv_y+zv_z)=0$
implica que $v$ es tangente a $S$.
...
es introduciendo en cada  punto $p \in M$ un tensor de tipo (2,0), simétrico
$[\mathbf{g(u,v) = g(v,u) \forall u,v} \in T_pM]$ y no degenerado $[\mathbf{g(u,v) = 0
\forall v} \in T_pM \Ra \mathbf{u=0}]$.
Si además pedimos que este tensor sea
definido positivo $[\mathbf{g(u,u) \geq 0 (=\;\;\Sii\;\; u=0)}] $
...
\espa
\ejer: Pruebe que la distancia $L(\gamma) $ es independiente del parámetro
elegido.
...
tensor de tipo $(2,0)$ simétrico $\ve{g^{-1}}$ tal que 
\beq
\ve{g}(\ve{g^{-1}}(\tita,\;),\;)=\tita
\eeq
...
vectores  y co-vectores o por ejemplo entre tensores tipo (0,2),
(2,0) o (1,1).
...
\espa
\ejer: ¿Cómo denotaría un tensor de tipo $(2,0)$ antisimétrico?
...
_{b_1\ldots b_l}$ son tensores de tipo $(k,l)$ y $\alpha\in \re$  luego
\beq
...
\beq
\na_e \lp \delta^i{}_j  A^{a_1\ldots,j,\ldots,a_k}_{b_1\ldots,i,\ldots,b_k}\rp 
= \delta^i{}_j \na_e A^{a_1\ldots,j,\ldots,a_k}_{b_1\ldots,i,\ldots,b_k}
\eeq
...
actúa en $A^{a_1\cdots a_k}_{b_1\ldots b_l}$ general el campo 
...
$\na_c w_a$
para cualquier $w_a$ luego $\na_c l^a$ es el tensor de tipo (1,1) tal que cuando
...
tendremos que $w_a-w'_a=\sum_{i} f^{i}\mu^{i}_a$ con
$f^{i}$ funciones suaves que se anulan en $p$. En $p$ tenemos
entonces
\beq \barr{rcl}
\bar\na_c\lp w_a-w'_a\rp-\na_c\lp w_a-w'_a\rp 
&=&\sum_{i}\bar\na_c \lp f^{i}\mu^{i}_a\rp-\sum_{i} \na_c\lp
f^{i}\mu^{i}_a\rp \\
&=& \sum_i \mu^{i}_a\lp\bar\na_c f^{i}-\na_c f^{i}\rp=0
\earr
\eeq
ya que por {\it iv)} $\bar \na_c $ y $\na_c$ deben actuar del 
mismo modo en escalares --y en particular en las $f^{i}$--. 
Esto muestra que
\beq
\lp\bar\na_c-\na_c\rp w'_a = \lp\bar\na_c-\na_c\rp w_a 
\eeq
...
Pero entonces $\lp\bar\na_c-\na_c\rp $ debe ser un tensor de tipo (1,2) 
que está esperando {\it comerse} una forma para darnos el tensor de 
tipo (0,2) $\lp\bar\na_c -\na_c\rp w_a.$ 
