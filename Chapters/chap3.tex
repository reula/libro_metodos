...
$a$, y como ya hemos visto  una variedad. Sea $(v^{x}x,v^{y},vz^{z})$ un vector
en $(x,y,z)\in\re^{3}$, luego la condición $\ve{df}(\ve{v})=2(xv^{x}+yv^{y}+zv^{z})=0$
implica que $\ve{v}$ es tangente a $S$.
...
Note que el sistema coordenado nos da también un conjunto de $n$ funciones privilegiadas, es decir, las componentes del mapa $\phi$ que define la carta, $\{x^{j}\}$, $j=1..n$, $f^{i}(p)=x^{i}\circ\fip$
Si aplicamos los vectores base a estas funciones obtenemos, 

\[
 \ve{x}_{i} (f^{j}) := \frac{\partial x^{j}}{\partial x^{i}}|_{\phi(p)} = \delta^{j}_{i},
 \]
 %
 pero entonces como, $\ve{df}^{j}(\ve{x}_{i})=\ve{x}_{i} (f^{j})$, vemos que los diferenciales
 $ \ve{df}^{j}$ son la co-base de la base coordenada. 
 Usualmente se denotan estas componentes como $ \ve{dx}^{j}$.
