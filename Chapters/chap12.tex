...
\ejer:
Convénzase de que~\ron{ec2*} es la ecuación de equilibrio para el caso de que la fuerza aplicada sea lo suficientemente pequeña como para producir una deformación pequeña de la membrana.
...
$\partial \Omega$ no esté en el plano $z=0$, o -equivalentemente-  que esté
...
Este espacio es invariante con respecto a la acción del Laplaciano. 
...
Repitiendo la demostración anterior concluimos que 
...
\eeq
%
y que $\lam_1$, será un autovalor, es
...
La condición de que $\mbox{\cal Q}(u) := \langle u , -L(u) \rangle_{H^0(\Omega)}$ 
...
Note que dicha condición también nos asegura que el operador $L$ deja invariante los respectivos subespacios $H(i)$ que es necesario 
considerar en la demostración anterior.
